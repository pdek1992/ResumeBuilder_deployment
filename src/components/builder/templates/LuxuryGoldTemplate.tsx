...
function PreviewHeading({ children, accent }: { children: React.ReactNode; accent: string }) {
  return (
    <div className="mb-4 border-b pb-2" style={{ borderColor: accent , WebkitPrintColorAdjust: "exact" }}>
      <h3 className="font-serif text-[14px] font-bold uppercase tracking-[0.16em]" style={{ color: accent }}>{children}</h3>
    </div>
  );
}

export function LuxuryGoldTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  ...
  return (
    <div 
      ...
      data-template-layout="luxury-gold"
    >
      <div className="p-10 text-center" style={{ backgroundColor: `${accent}08` }}>
        {resume.personal.profilePhotoUrl && (
          <div className="mx-auto mb-6 h-32 w-32 overflow-hidden rounded-full border-4 border-white shadow-2xl">
            <img src={resume.personal.profilePhotoUrl} alt={fullName} className="h-full w-full object-cover" />
          </div>
        )}
        <h1 className="text-[38px] font-serif font-black tracking-tight text-slate-950">{fullName}</h1>
        <p className="mt-3 text-[14px] font-serif font-bold uppercase tracking-widest" style={{ color: accent }}>
      ...
      <div className="flex-1 px-8 py-10 md:px-12 space-y-9">
        {resume.summary && (
          <div>
