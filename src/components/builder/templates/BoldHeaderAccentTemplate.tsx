...
export function BoldHeaderAccentTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  ...
  return (
    <div 
      ...
      data-template-layout="bold-header-accent"
    >
      <div className="p-10 text-center" style={{ backgroundColor: accent }}>
        {resume.personal.profilePhotoUrl && (
          <div className="mx-auto mb-6 h-28 w-28 overflow-hidden rounded-full border-4 shadow-2xl border-white/20">
            <img src={resume.personal.profilePhotoUrl} alt={fullName} className="h-full w-full object-cover" />
          </div>
        )}
        <h1 className="text-[36px] font-black tracking-tight text-white">{fullName}</h1>
        <p className="mt-3 text-[14px] font-bold uppercase tracking-[0.35em] text-white/90">
          {resume.personal.headline || resume.ats.targetRole}
        </p>
        <div className="mt-8 flex flex-wrap justify-center gap-x-8 gap-y-3 text-[11px] font-semibold text-white/80 uppercase tracking-wider">
      ...
      <div className="flex-1 px-8 py-10 md:px-12 space-y-8">
        {resume.summary && (
          <div>
