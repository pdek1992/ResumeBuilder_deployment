...
function PreviewHeading({ children, accent }: { children: React.ReactNode; accent: string }) {
  return (
    <div className="mb-4 flex items-center gap-3">
      ...
      <h3 className="font-black uppercase text-[15px] tracking-[0.2em]" style={{ color: accent }}>{children}</h3>
    </div>
  );
}

export function CreativeDesignerSplitTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  ...
  return (
    <div 
      ...
      data-template-layout="creative-designer-split"
    >
      {/* SIDEBAR */}
      <div 
      ...
      >
        <div className="space-y-10 flex-1">
          ...
          {resume.projects.length > 0 && resume.projects.some(p => p.name) && (
            <div>
              <PreviewHeading accent="#ffffff">Projects</PreviewHeading>
                  ...
                  {item.link && <a href={item.link} className="mt-1 text-[10px] underline text-slate-400 block">{item.link}</a>}
                    ...
                    <div className="mt-2 space-y-2">
                      {item.highlights.filter(Boolean).map((highlight, index) => (
                        <p key={index} className="leading-relaxed text-white/80 break-words whitespace-pre-wrap flex gap-3 text-[11.5px]">
          ...
          <div className="flex items-start justify-between gap-6">
            <div className="flex-1">
              <div className="flex items-center gap-6">
                {resume.personal.profilePhotoUrl && (
                  <div className="shrink-0 overflow-hidden shadow-2xl h-32 w-32 rounded-full">
                    <img src={resume.personal.profilePhotoUrl} alt={fullName} className="h-full w-full object-cover" />
                  </div>
                )}
                <div>
                  <h1 className="font-display font-black leading-tight tracking-tight break-words text-slate-950 text-[38px]">{fullName}</h1>
            ...
            {template.icon && !resume.personal.profilePhotoUrl && (
              <div className="hidden h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-white/10 backdrop-blur-sm md:flex">
                <img src={template.icon} alt="" className="h-12 w-12 contrast-125 opacity-40 invert brightness-0" />
              </div>
            )}
          </div>
        </div>

        <div className="flex-1 px-8 pb-10 md:px-12 space-y-10">
          {resume.summary && (
            <div>
