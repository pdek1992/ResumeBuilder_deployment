...
export function DeepCharcoalTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  ...
  return (
    <div 
      ...
      data-template-layout="deep-charcoal"
    >
      {/* SIDEBAR */}
      <div 
      ...
      >
        <div className="mb-10">
          {resume.personal.profilePhotoUrl ? (
            <div className="mx-auto overflow-hidden shadow-xl border-4 h-28 w-28 rounded-full border-white/20">
              <img src={resume.personal.profilePhotoUrl} alt={fullName} className="h-full w-full object-cover" />
            </div>
          ) : (
            <div className="mx-auto flex items-center justify-center bg-white shadow-sm h-24 w-24 rounded-full bg-white/10">
        ...
        <div className="space-y-8 flex-1">
          {resume.skills.length > 0 && (
            <div>
              <PreviewHeading accent="#f3f4f6">Skills</PreviewHeading>
              <div className="mt-4 flex flex-wrap gap-2">
                {resume.skills.map((skill) => (
                  <span key={skill} className="rounded-full border px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide border-white/20 bg-white/10 text-[#f3f4f6]">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          {resume.education.length > 0 && (
            <div>
              <PreviewHeading accent="#f3f4f6">Education</PreviewHeading>
              <div className="mt-4 space-y-5">
                {resume.education.map(item => (
                  <div key={item.id}>
                    <div className="flex justify-between items-start gap-4">
                      <p className="text-[13.5px] font-black leading-tight text-white">{item.degree || "Degree"}</p>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] shrink-0 text-[#f3f4f6]/60">{item.endDate}</p>
                    </div>
                    <p className="mt-1.5 text-[11.5px] font-bold uppercase tracking-wide text-[#f3f4f6]/80">{item.school || "University"}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {!isPrintMode && (
          <div className="mt-auto pt-8 border-t border-white/10">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#f3f4f6]/40">High Fidelity Resume</p>
          </div>
        )}
      </div>
