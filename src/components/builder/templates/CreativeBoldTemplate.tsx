...
export function CreativeBoldTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  ...
  return (
    <div 
      ...
      data-template-layout="creative-bold"
    >
      <div 
        className="px-8 py-8 md:px-12 flex justify-between items-end mb-8"
        style={{ backgroundColor: accent }}
      >
        <div className="flex items-center gap-6">
          {resume.personal.profilePhotoUrl && (
            <div className="shrink-0 overflow-hidden shadow-2xl h-24 w-24 rounded-3xl border-4 border-white/20">
              <img src={resume.personal.profilePhotoUrl} alt={fullName} className="h-full w-full object-cover" />
            </div>
          )}
          <div>
            <h1 className="text-[38px] font-black tracking-tight text-white">{fullName}</h1>
            <p className="mt-1 text-[14px] font-bold uppercase tracking-widest text-white/90">
              {resume.personal.headline || resume.ats.targetRole}
            </p>
          </div>
        </div>
        <div className="text-right space-y-1 text-[11px] font-medium text-white/80">
      ...
      <div className="flex-1 px-8 py-10 md:px-12 space-y-10">
        {resume.summary && (
          <div>
