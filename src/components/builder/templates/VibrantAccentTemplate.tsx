...
function PreviewHeading({ children, accent }: { children: React.ReactNode; accent: string }) {
  return (
    <h3 className="text-[11px] font-black uppercase tracking-[0.28em] mb-4" style={{ color: accent }}>
      {children}
    </h3>
  );
}

export function VibrantAccentTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  ...
  return (
    <div 
      ...
      data-template-layout="vibrant-accent"
    >
      {/* SIDEBAR */}
      <div 
        className="shrink-0 p-8 flex flex-col bg-slate-50 border-r border-slate-100 w-[32%]"
        style={{ backgroundColor: `${accent}12` }}
      >
        <div className="mb-10">
          {resume.personal.profilePhotoUrl ? (
            <div className="mx-auto overflow-hidden shadow-xl border-4 h-28 w-28 rounded-3xl border-white/20">
        ...
        {!isPrintMode && (
          <div className="mt-auto pt-8 border-t opacity-20" style={{ borderColor: "rgba(0,0,0,0.1)" }}>
      ...
      <div className="flex-1 flex flex-col min-h-full">
        <div className="px-8 py-10 md:px-12 mb-8 bg-transparent">
          <div className="flex items-start justify-between gap-6">
            <div className="flex-1">
              <div>
                <h1 className="font-display font-black leading-tight tracking-tight break-words text-slate-950 text-[30px]">{fullName}</h1>
        ...
        <div className="flex-1 px-8 pb-10 md:px-12 space-y-6">
          {resume.summary && (
            <div>
