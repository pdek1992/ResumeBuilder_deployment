...
function PreviewHeading({ children, accent }: { children: React.ReactNode; accent: string }) {
  return (
    <div className="mb-4 flex items-center gap-3">
      <span className="h-6 w-1.5 shrink-0 rounded-full" style={{ backgroundColor: accent , WebkitPrintColorAdjust: "exact" }} />
      <h3 className="text-[11px] font-black uppercase tracking-[0.28em]" style={{ color: accent }}>{children}</h3>
    </div>
  );
}

export function InfographicSplitTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  ...
  return (
    <div 
      ...
      data-template-layout="infographic-split"
    >
      {/* SIDEBAR */}
      <div 
        className="shrink-0 p-8 flex flex-col w-[32%]"
        style={{ backgroundColor: accent, color: "#ffffff" }}
      >
        <div className="mb-10">
          {resume.personal.profilePhotoUrl ? (
            <div className="mx-auto overflow-hidden shadow-xl border-4 h-32 w-32 rounded-[2rem] border-white/20">
            ...
            <div className="mx-auto flex items-center justify-center bg-white shadow-sm h-24 w-24 rounded-2xl bg-white/10">
              <img src={template.icon || "/icons/icon-resume.png"} alt="Icon" className="h-10 w-10 opacity-10" />
            </div>
          )}
        </div>
        ...
        <div className="space-y-8 flex-1">
          {resume.skills.length > 0 && (
            <div>
              <PreviewHeading accent="#ffffff">Skills</PreviewHeading>
              <div className="mt-4 flex flex-wrap gap-2">
                {resume.skills.map((skill) => (
                  <span key={skill} className="rounded-full border px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide border-white/20 bg-white/10 text-white">
                    {skill}
                  </span>
                ))}
              </div>
          ...
          {resume.education.length > 0 && (
            <div>
              <PreviewHeading accent="#ffffff">Education</PreviewHeading>
              <div className="mt-4 space-y-5">
                {resume.education.map(item => (
                  <div key={item.id}>
                    <div className="flex justify-between items-start gap-4">
                      <p className="text-[13.5px] font-black leading-tight text-white">{item.degree || "Degree"}</p>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] shrink-0 text-white/60">{item.endDate}</p>
                    </div>
                    <p className="mt-1.5 text-[11.5px] font-bold uppercase tracking-wide text-white/80">{item.school || "University"}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {!isPrintMode && (
          <div className="mt-auto pt-8 border-t border-white/10">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">High Fidelity Resume</p>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col min-h-full">
        <div className="px-8 py-10 md:px-12 mb-8 bg-transparent">
          <div className="flex items-start justify-between gap-6">
            <div className="flex-1">
              <div>
                <h1 className="font-display font-black leading-tight tracking-tight break-words text-slate-950 text-[34px]">{fullName}</h1>
              ...
              <div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 border-t pt-6 text-[11px] font-medium tracking-wide border-slate-100 text-slate-500">
                {[resume.personal.location, resume.personal.phone, resume.personal.email, resume.personal.linkedIn ? "LinkedIn" : ""].filter(Boolean).map((text, i) => (
        ...
        <div className="flex-1 px-8 pb-10 md:px-12 space-y-8">
          {resume.summary && (
            <div>
            ...
            <div>
              ...
              <div className="mt-4 space-y-5">
                {resume.experience.map((item) => (
                  <div key={item.id} className="relative break-inside-avoid pl-6">
