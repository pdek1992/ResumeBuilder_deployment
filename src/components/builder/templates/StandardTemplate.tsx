...
function PreviewHeading({ children, accent }: { children: React.ReactNode; accent: string }) {
  return <h3 className="mb-4 text-[11px] font-black uppercase tracking-[0.28em]" style={{ color: accent , WebkitPrintColorAdjust: "exact" }}>{children}</h3>;
}

export function StandardTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  const fullName = [resume.personal.firstName, resume.personal.lastName].filter(Boolean).join(" ") || "Your Name";
  const accent = resume.style.accent || template.config_json.accent || "#000000";
  ...
  return (
    <div 
      ...
      data-template-layout={template.config_json.layout || "standard"}
    >
          ...
          <div className="flex-1">
            <div className="flex items-center gap-6">
              {resume.personal.profilePhotoUrl && (
                <div className="shrink-0 overflow-hidden shadow-2xl h-24 w-24 rounded-3xl border-4 border-white">
