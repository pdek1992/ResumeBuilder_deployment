...
export function ImpactfulBoldTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  ...
  return (
    <div 
      ...
      data-template-layout="impactful-bold"
    >
      <div className="px-8 py-10 md:px-12 mb-8" style={{ backgroundColor: `${accent}15` }}>
        <div className="flex items-start justify-between gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-6">
              {resume.personal.profilePhotoUrl && (
                <div className="shrink-0 overflow-hidden shadow-2xl h-24 w-24 rounded-full">
                  <img src={resume.personal.profilePhotoUrl} alt={fullName} className="h-full w-full object-cover" />
                </div>
              )}
              <div>
                <h1 className="font-display font-black leading-tight tracking-tight break-words text-slate-950 text-[42px]">{fullName}</h1>
                <p className="mt-2 font-bold uppercase tracking-widest break-words text-[14px]" style={{ color: accent }}>
          ...
          <div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {resume.skills.map((skill) => (
                <span key={skill} className="rounded bg-slate-900 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wide text-white">
                  {skill}
                </span>
              ))}
            </div>
