...
export function StartupMetricsTemplate({ resume, template, isPrintMode }: ResumePreviewProps) {
  ...
  return (
    <div 
      ...
      data-template-layout="startup-metrics"
    >
      <div 
        className="px-8 py-8 md:px-12 border-b border-slate-100 flex justify-between items-end mb-8"
        style={{ backgroundColor: `${accent}06` }}
      >
        <div className="flex items-center gap-6">
          {resume.personal.profilePhotoUrl && (
            <div className="shrink-0 overflow-hidden shadow-2xl h-20 w-20 rounded-3xl border-4 border-white">
              <img src={resume.personal.profilePhotoUrl} alt={fullName} className="h-full w-full object-cover" />
            </div>
          )}
          <div>
            <h1 className="text-[34px] font-black tracking-tight text-slate-900">{fullName}</h1>
