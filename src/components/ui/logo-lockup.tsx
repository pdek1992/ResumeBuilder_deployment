...
export function LogoLockup({ href = "/", className }: { href?: string; className?: string }) {
  return (
    <Link href={href} className={cn("flex items-center gap-3", className)}>
      <span className="flex h-14 w-14 items-center justify-center rounded-3xl bg-white shadow-soft-card">
        <Image src="/logo.png" alt="VigilSiddhi Logo" width={40} height={40} priority />
      </span>
