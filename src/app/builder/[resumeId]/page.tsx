...
import { getActiveResumePass, getAvailableMockInterviewPayment, hasInterviewGuideAccess } from "@/lib/payments/access";
...
export default async function ResumeEditorPage({ params }: { params: Promise<{ resumeId: string }> }) {
  ...
  const [resume, templates, activePass, mockInterviewCredit, interviewGuideAccess] = await Promise.all([
    getResumeForUser(profile.id, resumeId),
    listTemplates(),
    getActiveResumePass(profile.id),
    getAvailableMockInterviewPayment(profile.id),
    hasInterviewGuideAccess(profile.id),
  ]);
  ...
  return (
    <ResumeEditor
      ...
      hasInterviewGuideAccess={Boolean(interviewGuideAccess)}
    />
  );
}
